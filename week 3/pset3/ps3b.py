...
class Patient(object):
    ...
    def update(self):
        ...
        self.density = len(self.viruses) / self.getMaxPop()
        if self.density <= 1:
            for virus in self.getViruses():
                try:
                    self.viruses.append(virus.reproduce(self.density))
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for sim in range(numTrials):
        ...
        for i in range(300):
            ...
            virus_list[i] += patient.getTotalPop()

    avgVirusList = [ x / float(numTrials) for x in virus_list]
...
#simulationWithoutDrug(100, 1000, .99, 0.05, 2) # viruses grow quickly
